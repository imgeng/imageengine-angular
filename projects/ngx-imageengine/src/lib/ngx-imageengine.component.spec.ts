...
@Component({
  ...
  template: `<div>
    <ngx-imageengine
      [attr.directives]="directives_props"
      path="testing.jpg"
...
export class TestNgChangesComponent {
  @ViewChild(NgxImageengineComponent)
  public ie_component?: NgxImageengineComponent;
  @Input() directives_props: IEDirectives = { format: "jpg" };
}

function build_url(
  component: NgxImageengineComponent,
...
describe("NgxImageengineComponent", () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [NgxImageengineComponent, TestNgChangesComponent],
    }).compileComponents();
  });
  ...
  it("final src url should reflect both host + path", () => {
    ...
    expect(img.getAttribute("src")).toBe(
      build_url(component, { fit: "cropbox" })
    );
  });
  ...
  it("final src url should reflect both host + path", () => {
    ...
    component.wrapper = new MockElementRef(element, rect);
    component.directives = { fit: "outside", force_download: true };
    component.ngAfterViewInit();
    fixture.detectChanges();

    const img = fixture.nativeElement.querySelector(".ngx-ie-image");
    expect(img.getAttribute("src")).toBe(
      build_url(component, { fit: "outside", force_download: true })
    );
  });
  ...
  it("should derive the width but not height (because it's bigger than height) when derive_size is true and the wrapper has dimensions and no fitting is defined", () => {
    ...
    const img = fixture.nativeElement.querySelector(".ngx-ie-image");
    expect(img.getAttribute("src")).toBe(
      build_url(component, { width: 300, height: 200 })
    );
